...
![PDC11](PDC11_heatmap_resized.png){#fig-surus}
...
ss_H2009 <- readRDS("data/ss_H2009.rds")
...
```{r}
ss_H358 <- readRDS("data/ss_H358.rds")
...
```{r}
ss_PDC11_batch2 <- readRDS("data/ss_PDC11_batch2.rds")
...
Combine the beta values for those sites from all the cell lines:

```{r}
betas_combined <- cbind(betas_H2009[common_cgsites,],betas_H358[common_cgsites,])
```
